...
mongoose.connect(dbUrl, {useNewUrlParser: true}, function(err){
    if (err) {
        ...
        Account.find({}, function(err, results) {
            if (results && results.length) {
                results.forEach(function(result) {
                    result.accountId = result.accountId + 'temp';
        ...
        Session.find({}, function(err, results) {
            if (results && results.length) {
                results.forEach(function(result) {
                    result.accountAId = result.accountAId + 'temp';
                    result.accountBId = result.accountBId + 'temp';
                    if (result.messages && result.messages.length) {
                        result.messages.forEach(function(item, i) {
                            item.accountId = item.accountId + 'temp';
                        });
                    }
                    result.save(function(err) {
                        if (!err) {
